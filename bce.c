...
void StoreParams(char *systemFileName, global_broadcast_params_t gbp,
...
void LoadParams(char *systemFileName, global_broadcast_params_t *gbp,
                broadcast_system_t *sys) {
    ...
    char buf[4096];
    fread(buf, 1, 4096, params);
    pairing_init_set_str(p->pairing, buf);
    fclose(params);
...
void DecryptKEM_using_product(global_broadcast_params_t gbp, priv_key_t mykey,
                              element_t key, ct_t myct) {
    if (!gbp) {
        printf("ACK!  You gave me no broadcast params!  I die.\n");
        return;
    }
...
void Decrypt_BC_KEM_using_bitvect(global_broadcast_params_t gbp,
                                  priv_key_t mykey, element_t key, ct_t myct,
                                  char *recip) {
    if (!mykey) {
...
void BroadcastKEM_using_bitvec(global_broadcast_params_t gbp,
                               broadcast_system_t sys, char *recip, ct_t myct,
                               element_t key) {
...
void Change_decr_prod_indicies(global_broadcast_params_t gbp, int receiver,
    ...
    if (rems) {
        for (i = 0; i < N_rems; i++) {
            ...
            if (incl_num == receiver) {
                if (DEBUG)
                    printf("incl_num == receiver, continuing\n");
                continue;
            }
    ...
    if (adds) {
        for (i = 0; i < N_adds; i++) {
            // adding elements to the set after checking if they're already
            // there
            incl_num = adds[i];
            if (incl_num < 1 || incl_num > gbp->num_users) {
                printf("element %d was outside the range of valid users\n", i);
...
void Gen_decr_prod_from_indicies(global_broadcast_params_t gbp, int receiver,
    ...
    if (!mykey) {
        printf(
            "ACK!  You gave me no private key to put decryption product in.\n");
        return;
    }
    if (!in_recip) {
...
void Gen_decr_prod_from_bitvec(global_broadcast_params_t gbp, int receiver,
...
void Change_encr_prod_indicies(global_broadcast_params_t gbp,
                               broadcast_system_t sys, int *adds, int N_adds,
                               int *rems, int N_rems) {
    // REMOVES THE OLD ONES, THEN ADDS THE NEW ONES
    int i;
    element_t temp_inv;
    ...
    if (rems) {
        for (i = 0; i < N_rems; i++) {
            // removing elements from the set after checking if it's in the
            // bit-vector
            incl_num = rems[i];
            if (incl_num < 1 || incl_num > gbp->num_users) {
                printf("element %d was outside the range of valid users\n", i);
...
void Gen_encr_prod_from_indicies(global_broadcast_params_t gbp,
                                 broadcast_system_t sys, int *in_recip,
                                 int num_recip) {
    if (!gbp) {
        printf("ACK!  You gave me no broadcast params!  I die.\n");
        return;
        ...
        for (j = 0; j < 8; j++) {
            if (working & 1) {
                if (!already_set) {
                    ...
                    element_mul(sys->encr_prod, sys->encr_prod,
                                gbp->gs[n - main_index]);
                }
            }
            main_index++;
...
void Get_priv_key(global_broadcast_params_t gbp, broadcast_system_t sys, int i,
                  priv_key_t mykey) {
    if (!gbp) {
        printf("ACK!  You gave me no broadcast params!  I die.\n");
        return;
...
void Setup_global_broadcast_params(global_broadcast_params_t *sys,
    ...
    char buf[4096];
    fread(buf, 1, 4096, curveFile);
    pairing_init_set_str(gbs->pairing, buf);
    fclose(curveFile);
